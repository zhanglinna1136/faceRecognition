#原始版本，摄像头捕捉到的人脸特征点传不到calculateDistance函数内
import sys
import cv2
        ...
        while True:
            ...
            if (faceCheck != False):#进行人脸检测
                ...
                if result is None:
                    ...
                    print("qiandaochenggong1")
                    print(result)
                    img=result[0]
                    print(type(img))#img类型为<class 'numpy.ndarray'>
                    Y,X=img.shape[:2]
                    ...
                    pixmap = QtGui.QPixmap.fromImage(qimage)#pixmap类型为<class 'PyQt5.QtGui.QPixmap'>
                    a=pixmap.scaled(320,240)#调整图片显示的大小
                    self.photo.setPixmap(a)
