...
def noset_read_image_gbk(bgr_image, resize_height=None, resize_width=None, normalization=False,colorSpace='RGB'):
    '''
    ...
    #     data = f.read()  # 以二进制数据方式读入图片
    #     data = np.asarray(bytearray(data), dtype="uint8")  # 将data列表转为数组，类型为无符号整数
    #     bgr_image = cv2.imdecode(data, cv2.IMREAD_COLOR)  # 将data中的数据转为图像格式
    # 或者：
    ...
    if len(bgr_image.shape) == 2:  # 若是灰度图则转为三通道
        ...
        bgr_image = cv2.cvtColor(bgr_image, cv2.COLOR_GRAY2BGR)
    if colorSpace == 'RGB':
        image = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2RGB)  # 输入图片格式是BGR的，所以将BGR转为RGB
    elif colorSpace == "BGR":
        image = bgr_image
    else:
        exit(0)
    ...
    image = resize_image(image, resize_height, resize_width)  # 把图片片调整为指定的宽和高
    image = np.asanyarray(image)
    if normalization:
        image = image_normalization(image)
    # show_image("src resize image",image)
...
def change_image(bgr_image, resize_height=None, resize_width=None, normalization=False, colorSpace='RGB'):
    '''
    改变图片格式为RGB、大小和归一化,读取图片数据,默认返回的是uint8,[0,255]
    :param filename:
    :param resize_height:
    :param resize_width:
    :param normalization:是否归一化到[0.,1.0]
